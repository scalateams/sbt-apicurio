...
object SchemaFileUtils {
  ...
  private def loadSchemaFile(file: File, logger: Logger): Option[SchemaFile] = {
    try {
      val content = Using(Source.fromFile(file, "UTF-8"))(_.mkString).get
      val hash = computeHash(content)
      val ext = file.getName.split("\\.").lastOption.getOrElse("")

      ArtifactType.fromExtension(ext) match {
        case Some(artifactType) =>
          Some(SchemaFile(file, content, hash, artifactType))
        case None =>
          logger.warn(s"Unknown artifact type for file: ${file.getName}")
          None
      }
    } catch {
      case ex: Exception =>
        logger.error(s"Failed to load schema file ${file.getAbsolutePath}: ${ex.getMessage}")
        None
    }
  }
   ...
   */
  def saveSchema(outputDir: File, dependency: ApicurioDependency, content: String, logger: Logger): Try[File] = {
    Try {
      ...
      logger.info(s"Downloaded schema: ${dependency.groupId}:${dependency.artifactId}:${dependency.version} -> $targetFile")
      targetFile
    }
  }
}
